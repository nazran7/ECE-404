...
parser.add_argument("src", help="name of the file which contains message")
...
def encrypt():
    ...
    body_bv = BitVector( rawbytes = body )
    key = get_encryption_key()

    round_keys = generate_round_keys(key)
    cipher_blocks = []

    body_blocks = [body_bv[x*64:x*64+64] for x in range(body_bv.size//64)]

    print(len(body_blocks))

    for bitvec in tqdm(body_blocks):
        if bitvec.size > 0:
    ...
    with open(args.dest, 'wb') as outfile:
        ...
        for block in cipher_blocks:
            hex = block.get_bitvector_in_hex()
