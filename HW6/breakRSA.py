...
def CRT(n_list, cipher_list):
    ...
    total += cipher_list[0] * \
        int(holder_bv.multiplicative_inverse(n_0_bv)) * int(holder_bv)
    # Iteration through the second n and second cipher block
    holder_bv.set_value(intVal=(big_n//n_list[1]))
    total += cipher_list[1] * \
        int(holder_bv.multiplicative_inverse(n_1_bv)) * int(holder_bv)
    # Iteration through the second n and second cipher block
    holder_bv.set_value(intVal=(big_n // n_list[2]))
    total += cipher_list[2] * \
        int(holder_bv.multiplicative_inverse(n_2_bv)) * int(holder_bv)
...
def breakEncrypt(inputF, p, q, outF):
    ...
    while (input_bv.more_to_read):
        ...
        result = exponentiation(message=int(working_bv),
                                exponent=e_as_int, modulus=(p*q))
...
def test():
    ...
    decryptAllThree(inputF_1='enc1.txt', inputF_2='enc2.txt',
                    inputF_3='enc3.txt', n_file='myN.txt', outF='after_decrypting_3.txt')
...
if __name__ == '__main__':
    if len(sys.argv) < 2:
        print("Need to use 'test' or one of the flags -e or -c")
        ...
        # encrypt
        if len(sys.argv) == 7:
            encryptThree(msgF=sys.argv[2], outF_1=sys.argv[3],
                         outF_2=sys.argv[4], outF_3=sys.argv[5], n_File=sys.argv[6])
        else:
            print("Encryption Usage: breakRSA.py -e <file_to_encrypt> <encrypted_output_1> <encrypted_output_2> <encrypted_output_3> <n_file> ")
    elif sys.argv[1] == '-c':
        # decrypt
        if len(sys.argv) == 7:
            decryptAllThree(inputF_1=sys.argv[2], inputF_2=sys.argv[3],
                            inputF_3=sys.argv[4], n_file=sys.argv[5], outF=sys.argv[6])
        else:
            print("Decryption Usage: breakRSA.py -c <file_to_decrypt_1> <file_to_decrypt_2> <file_to_decrypt_3> <n_file> <decrypted_output> ")
    else:
        print("Need to use 'test' or one of the flags -e or -c")
